...
/** Permission-based guard for enterprise sections. */
export function useRequirePermission(permission, fallback = "/dashboard") {
  const router = useRouter();
  const { isReady, isAuthenticated: authed, permissions = [] } = useAuth();
  ...
  }, [isReady, authed, permissions, permission, fallback, router]);

  return {
    isCheckingAuth: !isReady || !authed || (permission ? !permissions.includes(permission) : false),
  };
}
