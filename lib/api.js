...
export async function getDashboardData() {
  ...
  if (!response.ok) {
    const payload = await response.json().catch(() => ({}));
    throwNormalizedError(payload, "Failed to fetch dashboard data");
  }
...
export async function getQuarantine() {
  const response = await authFetch(`${BASE_URL}/quarantine`, {
    cache: "no-store",
  });

  if (!response.ok) {
    const payload = await response.json().catch(() => ({}));
    throwNormalizedError(payload, "Failed to fetch quarantine data");
  }
...
export async function getQuarantinePage({ offset = 0, limit = 50 } = {}) {
  ...
  const response = await authFetch(`${BASE_URL}/quarantine/paged?${params.toString()}`, {
    cache: "no-store",
  });
...
export async function getSyncJobs() {
  const response = await authFetch(`${BASE_URL}/sync/jobs`, {
    cache: "no-store",
  });

  if (!response.ok) {
    const payload = await response.json().catch(() => ({}));
    throwNormalizedError(payload, "Failed to fetch sync jobs");
  }
...
export async function createAnnotation(payload) {
  const response = await authFetch(`${BASE_URL}/annotations`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
  ...
  if (!response.ok) {
    const errPayload = await response.json().catch(() => ({}));
    throwNormalizedError(errPayload, "Failed to create annotation");
  }
  return response.json();
}

export async function getAnnotations({ recordId, status = "all", offset = 0, limit = 50 } = {}) {
  const params = new URLSearchParams({
  ...
  const response = await authFetch(`${BASE_URL}/annotations?${params.toString()}`, {
    cache: "no-store",
  });
  if (!response.ok) {
    const errPayload = await response.json().catch(() => ({}));
    throwNormalizedError(errPayload, "Failed to load annotations");
  }
  return response.json();
}

export async function updateAnnotation(annotationId, payload) {
  const response = await authFetch(`${BASE_URL}/annotations/${annotationId}`, {
    method: "PUT",
    headers: {
      "Content-Type": "application/json",
    },
  ...
  if (!response.ok) {
    const errPayload = await response.json().catch(() => ({}));
    throwNormalizedError(errPayload, "Failed to update annotation");
  }
  return response.json();
}

export async function getAnnotationHistory(annotationId) {
  const response = await authFetch(`${BASE_URL}/annotations/${annotationId}/history`, {
    cache: "no-store",
  });
  if (!response.ok) {
    const errPayload = await response.json().catch(() => ({}));
    throwNormalizedError(errPayload, "Failed to load annotation history");
  }
  return response.json();
}
...
export async function uploadCsvAndStartIngestion(file, { target = "quarantine" } = {}) {
  if (!(file instanceof File)) {
    throw new Error("Please select a CSV file");
  }
  const normalizedTarget = target === "golden" ? "golden" : "quarantine";
  const form = new FormData();
  form.append("file", file);
  const params = new URLSearchParams({ target: normalizedTarget });
  const response = await authFetch(`${BASE_URL}/ingestion/upload?${params.toString()}`, {
    method: "POST",
    body: form,
    timeoutMs: 300000,
  });
...
export async function getLineageAssetImpact(assetId) {
  const response = await authFetch(`${BASE_URL}/lineage/impact/${assetId}`, {
    cache: "no-store",
  });
  if (!response.ok) {
    const payload = await response.json().catch(() => ({}));
...
export async function postLineageImpactAnalyze({
  question = "",
  assetId = null,
  nodeKey = "",
  field = "",
  ...
  if (nodeKey.trim()) {
    body.node_key = nodeKey.trim();
  }
  if (field.trim()) {
    body.field = field.trim();
  }
  const response = await authFetch(`${BASE_URL}/lineage/impact/analyze`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(body),
    timeoutMs: 90000,
...
export async function postClassificationAnalyzeField({ fieldName, datasetId = null } = {}) {
  const body = { field_name: fieldName ?? "" };
  if (datasetId != null) {
    body.dataset_id = datasetId;
  }
  const response = await authFetch(`${BASE_URL}/classification/analyze-field`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(body),
    timeoutMs: 60000,
  ...
  if (!response.ok) {
    const payload = await response.json().catch(() => ({}));
    throwNormalizedError(payload, "Failed to classify field");
  }
  return response.json();
}

export async function postClassificationAnalyzeDataset(datasetId) {
  const response = await authFetch(`${BASE_URL}/classification/analyze-dataset`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  ...
  if (!response.ok) {
    const payload = await response.json().catch(() => ({}));
    throwNormalizedError(payload, "Failed to classify dataset");
  }
  return response.json();
}

export async function postGlossaryGenerateField({ fieldName, datasetId = null } = {}) {
  const body = { field_name: fieldName ?? "" };
  if (datasetId != null) {
    body.dataset_id = datasetId;
  }
  const response = await authFetch(`${BASE_URL}/glossary/generate`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(body),
    timeoutMs: 90000,
  ...
  if (!response.ok) {
    const payload = await response.json().catch(() => ({}));
    throwNormalizedError(payload, "Failed to generate glossary definition");
  }
  return response.json();
}

export async function postGlossaryGenerateDataset(datasetId) {
  const response = await authFetch(`${BASE_URL}/glossary/generate-dataset`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ dataset_id: datasetId }),
    timeoutMs: 120000,
  ...
  if (!response.ok) {
    const payload = await response.json().catch(() => ({}));
    throwNormalizedError(payload, "Failed to generate dataset glossary");
  }
  return response.json();
}

export async function getGlossaryEntries({ datasetId = null, fieldName = "" } = {}) {
  const params = new URLSearchParams();
  if (datasetId != null) {
    params.set("dataset_id", String(datasetId));
  }
  if (fieldName.trim()) {
    params.set("field_name", fieldName.trim());
  }
  const response = await authFetch(`${BASE_URL}/glossary/entries?${params.toString()}`, {
    cache: "no-store",
  });
  if (!response.ok) {
    const payload = await response.json().catch(() => ({}));
    throwNormalizedError(payload, "Failed to load glossary entries");
  }
  return response.json();
}

export async function postGlossarySave(payload) {
  const response = await authFetch(`${BASE_URL}/glossary/save`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload),
  });
  if (!response.ok) {
    ...
    throwNormalizedError(err, "Failed to save glossary entry");
  }
  return response.json();
}

export async function postDocumentationGenerate(datasetId) {
  const response = await authFetch(`${BASE_URL}/documentation/generate`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ dataset_id: datasetId }),
    timeoutMs: 120000,
  ...
  if (!response.ok) {
    const payload = await response.json().catch(() => ({}));
    throwNormalizedError(payload, "Failed to generate documentation");
  }
  return response.json();
}

export async function getDatasetDocumentation(datasetId) {
  const response = await authFetch(`${BASE_URL}/documentation/dataset/${datasetId}`, {
    cache: "no-store",
  });
  ...
  if (!response.ok) {
    const payload = await response.json().catch(() => ({}));
    throwNormalizedError(payload, "Failed to load documentation");
  }
  return response.json();
}

export async function postDocumentationSave(payload) {
  const response = await authFetch(`${BASE_URL}/documentation/save`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload),
  });
  if (!response.ok) {
    const err = await response.json().catch(() => ({}));
    throwNormalizedError(err, "Failed to save documentation");
  }
  return response.json();
}

export async function postRulesRecommend(datasetId) {
  const response = await authFetch(`${BASE_URL}/rules/recommend`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ dataset_id: datasetId }),
    timeoutMs: 120000,
  ...
  if (!response.ok) {
    const payload = await response.json().catch(() => ({}));
    throwNormalizedError(payload, "Failed to generate rule recommendations");
  }
  return response.json();
}

export async function postRulesRecommendField({ fieldName, datasetId = null } = {}) {
  const body = { field_name: fieldName ?? "" };
  if (datasetId != null) {
    body.dataset_id = datasetId;
  }
  const response = await authFetch(`${BASE_URL}/rules/recommend-field`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(body),
    timeoutMs: 90000,
  ...
  if (!response.ok) {
    const payload = await response.json().catch(() => ({}));
    throwNormalizedError(payload, "Failed to suggest field rules");
  }
  return response.json();
}

export async function getRuleRecommendations({ datasetId = null, fieldName = "", status = "" } = {}) {
  const params = new URLSearchParams();
  if (datasetId != null) {
    params.set("dataset_id", String(datasetId));
  }
  ...
  const response = await authFetch(`${BASE_URL}/rules/recommendations?${params.toString()}`, {
    cache: "no-store",
  });
  if (!response.ok) {
    const payload = await response.json().catch(() => ({}));
    throwNormalizedError(payload, "Failed to load rule recommendations");
  }
  return response.json();
}

export async function postRuleRecommendationApprove(recId) {
  const response = await authFetch(`${BASE_URL}/rules/recommendations/${recId}/approve`, {
    method: "POST",
  });
  if (!response.ok) {
    const payload = await response.json().catch(() => ({}));
    throwNormalizedError(payload, "Failed to approve rule");
  }
  return response.json();
}

export async function postRuleRecommendationReject(recId) {
  const response = await authFetch(`${BASE_URL}/rules/recommendations/${recId}/reject`, {
    method: "POST",
  });
  if (!response.ok) {
    const payload = await response.json().catch(() => ({}));
    throwNormalizedError(payload, "Failed to reject rule");
  }
  return response.json();
}

export async function postDocumentationExport(datasetId, format = "markdown") {
  const params = new URLSearchParams({ format });
  const response = await authFetch(
    `${BASE_URL}/documentation/export?${params.toString()}`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ dataset_id: datasetId }),
  ...
  if (!response.ok) {
    const payload = await response.json().catch(() => ({}));
    throwNormalizedError(payload, "Failed to export documentation");
  }
  return response.json();
}
...
export async function postCopilotChat(question, pageContext = null) {
  const body = { question: question ?? "" };
  if (pageContext && typeof pageContext === "object" && Object.keys(pageContext).length > 0) {
    body.page_context = pageContext;
  }
  const response = await authFetch(`${BASE_URL}/ai/copilot/chat`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    body: JSON.stringify(body),
    // Groq LLM + DB context can exceed the default 20s client timeout.
    timeoutMs: 90000,
  });
  if (!response.ok) {
    const payload = await response.json().catch(() => ({}));
    throwNormalizedError(payload, "Failed to get copilot response");
  }
  return response.json();
}

export async function postStewardshipExplain(stewardshipId) {
  const response = await authFetch(`${BASE_URL}/stewardship/explain`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ stewardship_id: stewardshipId }),
    timeoutMs: 90000,
  });
  if (!response.ok) {
    const payload = await response.json().catch(() => ({}));
    throwNormalizedError(payload, "Failed to explain stewardship failure");
  }
  return response.json();
}

export async function postStewardshipRemediate(stewardshipId) {
  const response = await authFetch(`${BASE_URL}/stewardship/remediate`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ stewardship_id: stewardshipId }),
    timeoutMs: 120000,
  ...
  if (!response.ok) {
    const payload = await response.json().catch(() => ({}));
    throwNormalizedError(payload, "Failed to generate remediation");
  }
  return response.json();
}

export async function getStewardshipRemediation(stewardshipId) {
  const response = await authFetch(`${BASE_URL}/stewardship/remediation/${stewardshipId}`, {
    cache: "no-store",
  });
  if (response.status === 404) {
    return null;
  ...
  if (!response.ok) {
    const payload = await response.json().catch(() => ({}));
    throwNormalizedError(payload, "Failed to load remediation");
  }
  return response.json();
}

export async function postStewardshipRemediationAccept(remediationId) {
  const response = await authFetch(`${BASE_URL}/stewardship/remediation/${remediationId}/accept`, {
    method: "POST",
  });
  if (!response.ok) {
    const payload = await response.json().catch(() => ({}));
    throwNormalizedError(payload, "Failed to accept remediation");
  }
  return response.json();
}

export async function postStewardshipRemediationReject(remediationId) {
  const response = await authFetch(`${BASE_URL}/stewardship/remediation/${remediationId}/reject`, {
    method: "POST",
  });
  if (!response.ok) {
    const payload = await response.json().catch(() => ({}));
    throwNormalizedError(payload, "Failed to reject remediation");
  }
  return response.json();
}

export async function postStewardshipRemediationResolve(remediationId) {
  const response = await authFetch(`${BASE_URL}/stewardship/remediation/${remediationId}/resolve`, {
    method: "POST",
  });
  if (!response.ok) {
    const payload = await response.json().catch(() => ({}));
    throwNormalizedError(payload, "Failed to resolve remediation");
  }
  return response.json();
}

export async function postStewardshipRemediationAssign(remediationId, ownerEmail) {
  const response = await authFetch(`${BASE_URL}/stewardship/remediation/${remediationId}/assign`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  ...
  if (!response.ok) {
    const payload = await response.json().catch(() => ({}));
    throwNormalizedError(payload, "Failed to assign steward");
  }
  return response.json();
}
...
export async function getGovernanceScore() {
  const response = await authFetch(`${BASE_URL}/governance/score`, {
    cache: "no-store",
    timeoutMs: 120000,
  });
  if (!response.ok) {
    const payload = await response.json().catch(() => ({}));
    throwNormalizedError(payload, "Failed to fetch governance score");
  }
  return response.json();
}

export async function getGovernanceDashboard() {
  const response = await authFetch(`${BASE_URL}/governance/dashboard`, {
    cache: "no-store",
    timeoutMs: 120000,
  });
  if (!response.ok) {
    const payload = await response.json().catch(() => ({}));
    throwNormalizedError(payload, "Failed to fetch governance dashboard");
  }
  return response.json();
}

export async function getGovernanceScoreForDataset(datasetId) {
  const response = await authFetch(`${BASE_URL}/governance/score/${datasetId}`, {
    cache: "no-store",
    timeoutMs: 120000,
  });
  if (!response.ok) {
    const payload = await response.json().catch(() => ({}));
    throwNormalizedError(payload, "Failed to fetch dataset governance score");
  }
  return response.json();
}

export async function getComplianceScore() {
  const response = await authFetch(`${BASE_URL}/compliance/score`, {
    cache: "no-store",
    timeoutMs: 120000,
  });
  if (!response.ok) {
    const payload = await response.json().catch(() => ({}));
    throwNormalizedError(payload, "Failed to fetch compliance score");
  }
  return response.json();
}

export async function getComplianceDashboard() {
  const response = await authFetch(`${BASE_URL}/compliance/dashboard`, {
    cache: "no-store",
    timeoutMs: 120000,
  });
  if (!response.ok) {
    const payload = await response.json().catch(() => ({}));
    throwNormalizedError(payload, "Failed to fetch compliance dashboard");
  }
  return response.json();
}
